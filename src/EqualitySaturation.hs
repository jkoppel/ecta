...
copyLiftedEnvToChild :: Path -> Int -> EqConstraints
copyLiftedEnvToChild ep childIdx = foldMap doCopyVar [1..envSize - 1]
  where
...
lambda :: Node -> Node
lambda n = Node [mkEdge "lambda" [anyEnv, n] (copyLiftedEnvToChild envPath 0)]
...
plus1 :: Node
plus1 = lambda (withEnvPropagation envPath 2 [useVar 0 At 0] $ plusRaw anyNode (intNode 1))
