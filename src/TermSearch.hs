...
tau = createGloballyUniqueMu (\n -> union ([appType n n, arrowType n n, var1, var2, var3, var4] ++ constructors ++ map (constructorToEdge n) usedConstructors))
  where
    ...
    usedConstructors = [("Pair", 2), ("List", 1), ("Monad", 1), ("StateT", 3)]
...
replicatorTau = createGloballyUniqueMu (\n -> union ([var1, var2] ++ map (Node . (:[]) . constructorToEdge n) usedConstructors))
  where
    ...
    usedConstructors = [("Pair", 2), ("Maybe", 1), ("List", 1)]
...
anyFunc = Node [f17, f33, f34, f35, f36]
...
reduceFullyAndLog = go 0
  where
    go i n = do putStrLn $ "Round " ++ show i ++ ": " ++ show (nodeCount n) ++ " nodes, " ++ show (edgeCount n) ++ " edges"
                hFlush stdout
                if i == 2 then putStrLn (renderDot $ toDot n) else return ()
                -- when (i == 21) $ do
                --   let Node es = n
